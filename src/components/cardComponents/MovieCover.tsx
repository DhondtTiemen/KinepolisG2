    ...
    <>
      {
        images?.map((cover) => (
          ...
          <div className="mr-4">
            <img className="rounded-md w-4/5 ml-4 mt-4 relative z-10" src={`${baseURL}${cover.url}`} alt={cover.mediaType}/>
            <img className="rounded-md  relative -top-28 blur-md" src={`${baseURL}${cover.url}`} alt={cover.mediaType}/>
          </div>
          : 
          ''
        ))
